...
class ResponseUtil {
     ...
     */
    private static final MediaType MEDIA_TYPE_JSON = MediaType.APPLICATION_JSON_TYPE
            .withCharset("UTF-8");
    ...
    /**
     * Create a response of {@code "200 OK"} with the given entity with the
     * given media format.
     */
    ...
    /**
     * Create a response of {@code "302 Found"} with {@code Location} header
     * having the given location.
     */
    public static Response location(String location) {
        return builder(Status.FOUND).header(HttpHeaders.LOCATION, location)
                .build();
    }

    /**
     * Create a response of {@code "400 Bad Request"} with the given entity
     * formatted in {@code "application/json;charset=UTF-8"}.
     */
    ...
    /**
     * Create a response of {@code "401 Unauthorized"} with the given entity
     * formatted in {@code "application/json;charset=UTF-8"} and with
     * {@code WWW-Authenticate} header having the value specified by
     * {@code challenge}.
     */
    public static Response unauthorized(String entity, String challenge) {
        return builder(Status.UNAUTHORIZED, entity, MEDIA_TYPE_JSON)
                .header(HttpHeaders.WWW_AUTHENTICATE, challenge).build();
    }

    /**
     * Create a response of {@code "403 Forbidden"} with the given entity
     * formatted in {@code "application/json;charset=UTF-8"}.
     */
    ...
    /**
     * Create a response of {@code "404 Not Found"} with the given entity
     * formatted in {@code "application/json;charset=UTF-8"}.
     */
    ...
    /**
     * Create a response of {@code "500 Internal Server Error"} with the given
     * entity formatted in {@code "application/json;charset=UTF-8"}.
     */
    ...
    private static ResponseBuilder builder(Status status) {
        return Response.status(status).cacheControl(CACHE_CONTROL)
     ...
     */
    private static ResponseBuilder builder(Status status, String entity,
            MediaType type) {
        return builder(status).entity(entity).type(type);
    }
