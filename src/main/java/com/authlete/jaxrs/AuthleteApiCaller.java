...
class AuthleteApiCaller {
     ...
     * Constructor with an implementation of {@link AuthleteApi} interface.
     */
    public AuthleteApiCaller() {
        mApi = AuthleteApiFactory.getDefaultApi();
    }

    /**
     * Create an {@link InternalServerErrorException} instance to indicate that
     * an Authlete API call failed.
     */
    private InternalServerErrorException apiFailure(String path,
            AuthleteApiException e) {
        // Error message.
        String message = String.format("Authlete %s API failed: %s", path,
                e.getMessage());
        ...
        if (e.getResponseBody() != null) {
            ...
            message = new StringBuilder(message).append(": ")
                    .append(e.getResponseBody()).toString();
        }
    ...
    /**
     * Create an {@link InternalServerErrorException} instance to indicate that
     * the value of {@code action} parameter contained in a response from an
     * Authlete API is unknown.
     */
    public InternalServerErrorException unknownAction(String path,
            Enum<?> action) {
        // Error message.
        String message = String.format(
                "Authlete %s API returned an unknown action: %s", path, action);
    ...
    /**
     * Create an {@link InternalServerErrorException} instance having a response
     * body.
     */
     ...
     */
    public AuthorizationResponse callAuthorization(
    ...
    private AuthorizationResponse callAuthorization(String parameters) {
        if (parameters == null) {
            ...
            // string.
            // 'null' is regarded as a caller's error. An empty string is
            // regarded
     ...
     */
    private AuthorizationFailResponse callAuthorizationFail(String ticket,
        ...
        AuthorizationFailRequest request = new AuthorizationFailRequest()
                .setTicket(ticket).setReason(reason);
     ...
     * Authlete's {@code /api/auth/authorization/fail} API.
     */
    private Response createAuthorizationFailResponse(String ticket,
            AuthorizationFailRequest.Reason reason) {
        // Call Authlete's /api/auth/authorization/fail API.
        AuthorizationFailResponse response = callAuthorizationFail(ticket,
                reason);
     ...
     * Authlete's {@code /api/auth/authorization/fail} API.
     */
    public WebApplicationException authorizationFail(String ticket,
            AuthorizationFailRequest.Reason reason) {
     ...
     */
    private AuthorizationIssueResponse callAuthorizationIssue(String ticket,
            String subject, long authTime, String acr,
        ...
        AuthorizationIssueRequest request = new AuthorizationIssueRequest()
                .setTicket(ticket).setSubject(subject).setAuthTime(authTime)
                .setAcr(acr).setProperties(properties).setScopes(scopes);
    ...
    /**
     * Issue an authorization code, an ID token and/or an access token. This
     * method calls Authlete's {@code /api/auth/authorization/issue} API.
     */
    public Response authorizationIssue(String ticket, String subject,
            long authTime, String acr, Map<String, Object> claims,
            Property[] properties, String[] scopes) {
        // Call Authlete's /api/auth/authorization/issue API.
        AuthorizationIssueResponse response = callAuthorizationIssue(ticket,
                subject, authTime, acr, claims, properties, scopes);
     ...
     */
    public TokenResponse callToken(MultivaluedMap<String, String> parameters,
            String clientId, String clientSecret, Property[] properties) {
        String params = URLCoder.formUrlEncode(parameters);
     ...
     * Call Authlete's {@code /api/auth/token} API.
     */
    private TokenResponse callToken(String parameters, String clientId,
            String clientSecret, Property[] properties) {
        if (parameters == null) {
            // Authlete returns different error codes for null and an empty
            // string.
            // 'null' is regarded as a caller's error. An empty string is
     ...
     */
    private TokenFailResponse callTokenFail(String ticket,
            TokenFailRequest.Reason reason) {
        // Create a request for /api/auth/token/fail API.
        TokenFailRequest request = new TokenFailRequest().setTicket(ticket)
                .setReason(reason);
    ...
    /**
     * Create a response that describes the failure. This method calls
     * Authlete's {@code /api/auth/token/fail} API.
     */
    private Response createTokenFailResponse(String ticket,
            TokenFailRequest.Reason reason) {
        // Call Authlete's /api/auth/token/fail API.
     ...
     * Authlete's {@code /api/auth/token/fail} API.
     */
    public WebApplicationException tokenFail(String ticket,
            TokenFailRequest.Reason reason) {
        // Create a response to the client application with the help of
    ...
    private TokenIssueResponse callTokenIssue(String ticket, String subject,
        ...
        TokenIssueRequest request = new TokenIssueRequest().setTicket(ticket)
                .setSubject(subject).setProperties(properties);
    ...
    /**
     * Issue an access token and optionally an ID token. This method calls
     * Authlete's {@code /api/auth/token/issue} API.
     */
     ...
     */
    public Response serviceJwksGet(boolean pretty, boolean includePrivateKeys)
     ...
     * Call Authlete's {@code /api/auth/revocation} API.
     */
    private RevocationResponse callRevocation(String parameters,
        ...
        RevocationRequest request = new RevocationRequest()
                .setParameters(parameters).setClientId(clientId)
    ...
    public UserInfoResponse callUserInfo(String accessToken) {
        ...
        UserInfoRequest request = new UserInfoRequest().setToken(accessToken);
    ...
    public Response userInfoIssue(String accessToken,
        ...
        case INTERNAL_SERVER_ERROR:
            // 500 Internal Server Error
            return ResponseUtil.bearerError(Status.INTERNAL_SERVER_ERROR,
                    content);
     ...
     */
    public IntrospectionResponse callIntrospection(String accessToken,
            String[] scopes, String subject) {
        // Create a request for /api/auth/introspection API.
        IntrospectionRequest request = new IntrospectionRequest()
                .setToken(accessToken).setScopes(scopes).setSubject(subject);
